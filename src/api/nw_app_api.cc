...
bool NwAppGetIEBookmarksFunction::RunAsync() {
  ...
  results_->Append(std::unique_ptr<base::Value>(static_cast<base::Value*>(values)));
  SendResponse(true);
  return true;
#else
  return false;
...
bool NwAppGetFlagsSettingFunction::RunAsync() {
...
bool NwAppSetFlagsSettingFunction::RunAsync() {
